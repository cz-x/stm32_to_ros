...
void MPU::measure_gyroscope()
{
	...
	raw_rotation.x = (gyro_buffer[0] << 8) | gyro_buffer[1];
	raw_rotation.y = (gyro_buffer[2] << 8) | gyro_buffer[3];
	raw_rotation.z = (gyro_buffer[4] << 8) | gyro_buffer[5];
}
...
void MPU::measure_acceleration()
{
	...
	raw_acceleration.x = (acc_buffer[0] << 8) | acc_buffer[1];
	raw_acceleration.y = (acc_buffer[2] << 8) | acc_buffer[3];
	raw_acceleration.z = (acc_buffer[4] << 8) | acc_buffer[5];
}
